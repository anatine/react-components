...
import { CalcOffsetFn, calcOffset } from '../helpers/dom';
...
export default function useClockEvents(
  ...
  const calcOffsetCache: React.MutableRefObject<null | CalcOffsetFn> =
    useRef(null);
  ...
  const handleMouseUp = useCallback(
    (e: MouseEvent) => {
      if (!clock.current || !calcOffsetCache.current) {
        return;
      }
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLElement>) => {
      ...
      handleMouseDrag(e as never); // TODO: Fix this time issue
    },
  ...
  const handleTouchStart = useCallback(
    (e: TouchEvent) => {
      ...
      document.addEventListener('touchend', handleStopDrag, { passive: true });
  ...
  useEffect(() => {
    ...
    if (currentTarget) {
      currentTarget.addEventListener(type, handleTouchStart, { passive: true });
    }
    return () => {
      if (currentTarget) {
